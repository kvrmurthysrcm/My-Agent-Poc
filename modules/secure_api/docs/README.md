# Secure API Gateway Docs

The current phase-wise implementation and validation guide lives in the module README:

```text
modules/secure_api/README.md
```

Use that README for:

- Phase 1 Keycloak validation
- Phase 2 wrapper health validation
- Phase 3 login, refresh, and logout validation
- Phase 4 JWT validation and `/auth/me` validation
- Current phase 5 gaps
- Test commands and expected responses
