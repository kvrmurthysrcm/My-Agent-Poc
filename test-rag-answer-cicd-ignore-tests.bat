...
echo COMMAND:
echo   docker run -d --name %LOCAL_CONTAINER% -p 8002:8002 %RUNTIME_IMAGE%
docker run -d --name "%LOCAL_CONTAINER%" -p 8002:8002 "%RUNTIME_IMAGE%"
if errorlevel 1 goto :fail
...
echo.
echo VERIFICATION COMMAND:
echo   curl.exe --fail http://localhost:8002/health
curl.exe --fail http://localhost:8002/health
if errorlevel 1 (
    echo.
...
echo.
echo Restarting Jenkins so the mounted kubeconfig is definitely refreshed.
echo COMMAND:
echo   docker restart %JENKINS_CONTAINER%
docker restart "%JENKINS_CONTAINER%"
if errorlevel 1 goto :fail

echo Waiting 12 seconds for Jenkins restart...
timeout /t 12 /nobreak >nul

echo.
echo VERIFICATION COMMAND:
echo   docker exec %JENKINS_CONTAINER% kubectl get nodes -o wide
docker exec "%JENKINS_CONTAINER%" kubectl get nodes -o wide
if errorlevel 1 goto :fail
